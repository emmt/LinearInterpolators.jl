...
immutable FlatLimits{T<:AbstractFloat} <: Limits{T}
    ...
    function (::Type{FlatLimits{T}}){T}(len::Integer)
        @assert len ≥ 1
        new{T}(len)
    end
end

limits{T,S}(::Kernel{T,S,Flat}, len::Integer) =
    FlatLimits{T}(len)

boundaries(::FlatLimits) = Flat

@inline function getcoefs{T}(ker::Kernel{T,1,Flat}, lim::FlatLimits{T}, x::T)
    r = round(x)
...
@inline function getcoefs{T}(ker::Kernel{T,2,Flat}, lim::FlatLimits{T}, x::T)
    f = floor(x)
    j1 = trunc(Int, f)
    j2 = j1 + 1
...
@inline function getcoefs{T}(ker::Kernel{T,3,Flat}, lim::FlatLimits{T}, x::T)
    r = round(x)
    j2 = trunc(Int, r)
    j1, j3 = j2 - 1, j2 + 1
...
@inline function getcoefs{T}(ker::Kernel{T,4,Flat}, lim::FlatLimits{T}, x::T)
    f = floor(x)
    j2 = trunc(Int, f)
