...
# interpolators.jl -
#
# Implement constructors and basic methods for interpolators.
#
#------------------------------------------------------------------------------
#
...
export LazyInterpolator, SparseInterpolator

using ..LinearInterpolators
using ..LinearInterpolators: to_ntuple, to_unit_range, compute_indices,
    check_indices, nth, argument_error, dimension_mismatch,
...
abstract type AbstractInterpolator{T,L,M,N,O,S} <: AbstractInterpolator{T,M,N} end

"""
    SparseInterpolator(pos, ker, cols, bnd=Flat())
...
and the `M`-tuple `rows` of the dimensions of the interpolation result:
    ...
    SparseInterpolator(xform, rows, ker, cols, bnd)
...
"""
function SparseInterpolator{T,L,M,N,O,S}(
    rows::NTuple{M,Integer},
    cols::NTuple{N,Integer},
    wgt::W,
    ind::I;
    inbounds::Bool=false) where {T,L,M,N,O,S,
        ...
        return new{T,L,M,N,O,S,W,I}(rows, cols, wgt, ind)
    end
end

"""
    LazyInterpolator{T,L,M,N,O,S}(...)
...
"""
function LazyInterpolator{T,L,M,N,O,S}(
    rows::NTuple{M,Integer},
    cols::NTuple{N,Integer},
    pos::P,
    ker::K,
    bnd::B) where {T,L,M,N,O,S,
    ...
    return new{T,L,M,N,O,S,P,K,B}(rows, cols, pos, ker, bnd)
end

"""
    nskip(A)

yields the number of unchanged leading dimensions by linear operator `A`.
This corresponds to the type parameter `L` of `A`.   Argument can also be
the type of a linear interpolator.

"""
nskip(A::AbstractInterpolator) = nskip(typeof(A))
nskip(::Type{<:AbstractInterpolator{T,L}) where {T,L} = L

"""
    cols(A)

yields the lenghts of interpolated dimensions in the input array for a linear
operator `A`.

"""
cols(A::AbstractInterpolator) = getfield(A, :cols)

"""
    ncols(A) -> prod(cols(A))

yields the product of the lenghts of interpolated dimensions in the input array
for a linear operator `A`.

"""
ncols(A::AbstractInterpolator) = prod(cols(A))

"""
    rows(A)

yields the lenghts of interpolated dimensions in the output array for a linear
operator `A`.

"""
rows(A::AbstractInterpolator) = getfield(A, :rows)

"""
    nrows(A) -> prod(rows(A))

yields the product of the lenghts of interpolated dimensions in the output
array for a linear operator `A`.

"""
nrows(A::AbstractInterpolator) = prod(rows(A))

function Base.show(io::IO,
                   A::SparseInterpolator{T,L,M,N,O,S}) where {T,L,M,N,O,S}
    print(io, "SparseInterpolator{$T,$L,$M,$N,$O,$S}: ")
    join(io, cols(A), "×")
    print(io, " → ")
    join(io, rows(A), "×")
end

function Base.show(io::IO, A::LazyInterpolator{T,L,M,N,O,S}) where {T,L,M,N,O,S}
    print(io, "LazyInterpolator{$T,$L,$M,$N,$O,$S}: ")
    join(io, cols(A), "×")
    print(io, " → ")
    join(io, rows(A), "×")
end
...
# Common constructors.
for func in (:SparseInterpolator, :LazyInterpolator)
    @eval begin
        ...
        function $func{T}(pos::AbstractArray{<:Real,M},
                          ...
                          bnd::Repeatable{BoundaryConditions,1} = Flat();
                          kwds...) where {T,M}
            return $func{T,Undef,M,1}(
                convert_coord($func{T,M,1}, pos),
                convert_kernel($func{T,M,1}, ker),
                Dims{1}(cols),
                to_ntuple(Tuple{BoundaryConditions}, bnd); kwds...)
        end
        function $func{T}(pos::Union{NTuple{N,AbstractArray{<:Real,M}},
                          ...
                          kwds...) where {T,M,N}
            return $func{T,Undef,M,N}(
                convert_coord($func{T,M,N}, pos),
                convert_kernel($func{T,M,N}, ker),
                Dims{N}(cols),
                to_ntuple(NTuple{N,BoundaryConditions}, bnd); kwds...)
        end

        # Parse keywords.
        function $func{T,Undef,M,N}(args...;
                                    skip::Int=0,
                                    order::Int=213) where {T,M,N}
            return $func{T,skip,M,N,order}(args...)
        end
    end
end
...
# called.
function SparseInterpolator{T,L,M,N,O}(
    pos::AbstractArray{<:NTuple{N,Real},M},
    ker::NTuple{N,Kernel{T}},
    cols::Dims{N},
    bnd::NTuple{N,BoundaryConditions}) where {T,L,M,N,O}
    ...
    return SparseInterpolator{T,L,M,N,O,S}(rows, cols, wgt, ind;
                                           inbounds=true)
end

function LazyInterpolator{T,L,M,N,O}(
    ...
    rows = size(pos)
    S = map(length, ker)
    return LazyInterpolator{T,L,M,N,O,S}(rows, cols, pos, ker, bnd)
end

# Conversion of element type.
for type in (:AbstractInterpolator, :AbstractInterpolator)
    @eval begin
        $type{T}(A::SparseInterpolator) where {T} =
            SparseInterpolator{T}(A)
        $type{T}(A::LazyInterpolator) where {T} =
            LazyInterpolator{T}(A)
    end
end

SparseInterpolator{T}(A::SparseInterpolator{T}) where {T} = A
function SparseInterpolator{T}(
    A::SparseInterpolator{T′,L,M,N,O,S}) where {T,T′,L,M,N,O,S}
    return SparseInterpolator{T,L,M,N,O,S}(
        rows(A), cols(A),
        ntuple(i -> with_eltype(Tuple{Vararg{T}}, A.wgt[i]), Val(N)),
        A.ind; inbounds=true)
end

LazyInterpolator{T}(A::LazyInterpolator{T}) where {T} = A
function LazyInterpolator{T}(
    A::LazyInterpolator{T′,L,M,N,O,S}) where {T,T′,L,M,N,O,S}
    return LazyInterpolator{T,L,M,N,O,S}(
...
function convert_coord(::Type{<:SparseInterpolator{T,M,1}},
                       pos::AbstractArray{<:Real,M}) where {T,M}
    return ZippedArray(pos)
end
function convert_coord(::Type{<:SparseInterpolator{T,M,N}},
...
# needed?
function convert_coord(::Type{<:LazyInterpolator{T,L,M,1}},
                       pos::AbstractArray{<:Real,M}) where {T,L,M}
    return ZippedArray(with_eltype(T, pos))
end
function convert_coord(::Type{<:LazyInterpolator{T,L,M,N}},
                       pos::NTuple{N,AbstractArray{<:Real,M}}) where {T,L,M,N}
    return ZippedArray(map(A -> with_eltype(T, A), pos)...)
end
function convert_coord(::Type{<:LazyInterpolator{T,L,M,N}},
                       pos::AbstractArray{<:NTuple{N,Real},M}) where {T,L,M,N}
    return with_eltype(NTuple{N,T}, pos)
end
function convert_coord(::Type{<:LazyInterpolator{T,L,M,N}},
                       pos::AbstractArray{<:NTuple{N,T},M}) where {T,L,M,N}
    return pos
end

# Convert argument specifying interpolation kernel(s) in constructors.
function convert_kernel(A::Type{<:AbstractInterpolator{T,L,M,N}},
                        ker::Kernel) where {T,L,M,N}
    ker_d = with_eltype(T, ker)
    return ntuple(d -> ker_d, Val(N))
end
function convert_kernel(A::Type{<:AbstractInterpolator{T,L,M,N}},
                        ker::NTuple{N,Kernel}) where {T,L,M,N}
    return ntuple(d -> with_eltype(T, ker[d]), Val(N))
end
function convert_kernel(A::Type{<:AbstractInterpolator{T,L,M,N}},
                        ker::NTuple{N,Kernel{T}}) where {T,L,M,N}
    return ker
end

# Build a sparse interpolator from a lazy version.
SparseInterpolator(A::LazyInterpolator) =
    SparseInterpolator(A.pos, A.ker, cols(A), A.bnd)
SparseInterpolator{T}(A::LazyInterpolator) where {T} =
    SparseInterpolator{T}(A.pos, A.ker, cols(A), A.bnd)
...
function instantiate!(wgt::AbstractArray{NTuple{S,T},M},
                      ...
                      pos::AbstractArray{<:NTuple{N,Real},M},
                      d::Int,
                      bnd::BoundaryConditions) where {T,L,M,N,O,S}
...
function LazyAlgebra.vcreate(::Type{LazyAlgebra.Direct},
                             A::AbstractInterpolator{Ta,La,Ma,Na},
                             x::AbstractArray{Tx,Nx},
                             scratch::Bool) where {Ta,La,Ma,Na, Tx,Nx}
    N = Nx - (La + Na)
    N ≥ 0 || dimension_mismatch("array argument have too few dimensions")
    T = promote_type(Ta, Tx)
    dims = size(x)
    return Array{T,N}(undef, dims[1:La]..., rows(A)..., dims[La+Na+1:end]...)
end
...
function LazyAlgebra.vcreate(::Type{LazyAlgebra.Adjoint},
    ...
    T = promote_type(Ta, Tx)
    dims = size(x)
    return Array{T,N}(undef, dims[1:La]..., cols(A), dims[La+Ma+1:end]...)
end

function LazyAlgebra.apply!(alpha::Number,
                            ::Type{LazyAlgebra.Direct},
                            A::AbstractInterpolator,
                            x::AbstractArray,
                            scratch::Bool,
                            beta::Number,
                            y::AbstractArray)
    check_axes(nskip(A), axes(x), cols(A), axes(y), rows(A))
    if alpha == zero(alpha)
        vscale!(y, beta)
...
@generated function unsafe_apply_direct!(
    ...
    y::AbstractArray{<:Any,M}) where {
        M,N,S,T<:AbstractInterpolator{<:Any,M,N,S}}
    return unsafe_apply_direct!(
        Expr, collect(S), Val(T <: LazyInterpolator))
end
...
function LazyAlgebra.apply!(alpha::Number,
                            ...
                            beta::Number,
                            y::AbstractArray)
    check_axes(nskip(A), axes(x), rows(A), axes(y), cols(A))
    vscale!(y, beta)
    if alpha != zero(alpha)
        α = promote_multiplier(alpha, promote_eltype(A, x))
...
function check_axes(L::Int,
                    inp_axes::NTuple{Ni,AbstractUnitRange{Int}},
                    inp_dims::Dims{Di},
                    out_axes::NTuple{No,AbstractUnitRange{Int}},
                    out_dims::Dims{Do}) where {No,Do,Ni,Di}
    Ni ≥ L + Di || dimension_mismatch("input array has too few dimensions")
    No ≥ L + Do || dimension_mismatch("output array has too few dimensions")
    Ni - Di == No - Do || dimension_mismatch(
        "input and output arrays have incompatible number of dimensions")
    for d in 1:La
        out_axes[d] == inp_axes[d] || dimension_mismatch(
            "input and output arrays have incompatible leading indices")
    end
    for d in L+1:Ni-Di
        out_axes[Do+d] == inp_axes[Di+d] || dimension_mismatch(
            "input and output arrays have incompatible trailing indices")
    end
    for d in 1:Di
        inp_axes[L+d] == Base.OneTo(inp_dims[d]) || dimension_mismatch(
            "incompatible interpolation indices in input array")
    end
    for d in 1:Do
        out_axes[L+d] == Base.OneTo(out_dims[d]) || dimension_mismatch(
            "incompatible interpolation indices in output array")
    end
end
...
# Generate code to extract or compute interpolation weights and indices at
# index i fo all interpolated dimensions.
function get_weights_and_indices(::Type{Expr},
                                 ::Val{:sparse},
...
"""
function unsafe_apply_direct!(::Type{Expr}, alg::Val, S::AbstractVector{Int})
    # Get weights and indices at index i for all interpolated dimensions.
    ex = get_weights_and_indices(Expr, alg, S)
...
"""
function unsafe_apply_adjoint!(::Type{Expr}, alg::Val, S::AbstractVector{Int})
    # Get weights and indices at index i for all interpolated dimensions.
    ex = get_weights_and_indices(Expr, alg, S)
